...
- (void)drawRect:(CGRect)rect
{
    ...
    for (BKXCoreTextImageData * imageData in self.data.imageArray) {
            if (imageData.name.length > 0) {
                    // 适配lj后台
                    if ([imageData.name containsString:@"base64"] || imageData.name.length > 1000) {
                        ...
                        UIImage *image = [UIImage imageWithData:imagebase];
    //                    image = imageData.baseImage;
                        CGContextDrawImage(context, imageData.imagePosition, image.CGImage);

                    }else{
                        SDImageCache * cache = [SDImageCache sharedImageCache];
                        UIImage *image = [cache imageFromDiskCacheForKey:imageData.name];
                        if (image) {
                            CGContextDrawImage(context, imageData.imagePosition, image.CGImage);
                        }else{
